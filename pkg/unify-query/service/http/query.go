...
func queryRawWithInstanceDirect(ctx context.Context, queryDirect structured.QueryDirect) (total int64, list []map[string]any, resultTableOptions metadata.ResultTableOptions, err error) {
	...
	go func() {
		...
		span.Set("queryDirect-ref-num", ref.Count())
		span.Set("result-data-num", len(data))

		query.SortSliceListWithTime(data, queryDirect.OrderBy.Orders(), fieldType)

		span.Set("queryDirect-scroll", queryDirect.Scroll)
		span.Set("queryDirect-result-table", queryDirect.ResultTableOptions)

		//scroll 和 searchAfter 模式不进行裁剪
		if queryDirect.Scroll == "" && !queryDirect.IsSearchAfter && queryDirect.ResultTableOptions.IsCrop() {
			// 判定是否启用 multi from 特性
			span.Set("queryDirect-multi-from", queryDirect.IsMultiFrom)
			span.Set("data-length", len(data))
			span.Set("queryDirect-ts-from", queryDirect.From)
			span.Set("queryDirect-ts-limit", queryDirect.Limit)
		...
		span.Set("queryDirect-label-map", allLabelMap)
		span.Set("queryDirect-highlight", queryDirect.HighLight)
	...
	go func() {
		...
		span.Set("queryDirect-ref-count", refCount)
