...
func TestGraphE2E(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			// Step 5: 使用 JSONEq 对每个图进行完整 JSON 比较
			for i, expectedGraph := range tc.ExpectedGraphs {
				expectedJSON, err := json.Marshal(expectedGraph)
				require.NoError(t, err, "Marshal expected graph[%d] failed", i)

				actualJSON, err := json.Marshal(actualGraphs[i])
				require.NoError(t, err, "Marshal actual graph[%d] failed", i)

				assert.JSONEq(t, string(expectedJSON), string(actualJSON), "LivenessGraph[%d] mismatch", i)
			}
		})
	}
}
